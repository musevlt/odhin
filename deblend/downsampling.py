    ...
    for i in range(dim[0]):
        for j in range(dim[1]):
            downsampleMatrix[max(i *
                             ...
                             1, j +
                             dim[0] *
                             i] = numba_outer(abundanceMap0[i, max(0, i *
                                                                   int(np.floor(di)) -
                                                                   1):(i +
                                                                       1) *
                                                            int(np.ceil(di)) +
                                                            1], abundanceMap1[j, max(0, j *
                                                                                     int(np.floor(dj)) -
                                                                                     1):(j +
                                                                                         1) *
                                                                              int(np.ceil(dj)) +
    ...
    for k, imHR in enumerate(cubeHR):
        for i in range(dim[0]):
            for j in range(dim[1]):
                ...
                cubeLR[k,
                       i,
                       j] = np.sum(numba_outer(abundanceMap0[i,
                                                             max(i * int(np.floor(di)) - 1,
                                                                 0):(i + 1) * int(np.ceil(di)) + 1],
                                               abundanceMap1[j,
                                                             max(0,
                                                                 j * int(np.floor(dj)) - 1):(j + 1) * int(np.ceil(dj)) + 1]) * imHR[max(0,
                                                                                                                                        i * int(np.floor(di)) - 1):(i + 1) * int(np.ceil(di)) + 1,
                                                                                                                                    max(0,
                                                                                                                                        j * int(np.floor(dj)) - 1):(j + 1) * int(np.ceil(dj)) + 1])

        downsampleMatrix = downsampleMatrix.reshape(imHR.size, cubeLR[0].size)
        # handle nan values
    cubeLR[np.where(np.isnan(cubeLR))] = np.median(
        cubeLR[np.where(np.isnan(cubeLR) == False)])
    if len(cubeHR.shape) == 2:
        cubeLR = cubeLR[0, :, :]
    if returnMatrix:
