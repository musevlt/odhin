...
@numba.jit
def downsampling(imHR,dim,returnMatrix=False):
    di=imHR.shape[0]/float(dim[0])
    dj=imHR.shape[1]/float(dim[1])
    
    abundanceMap0,abundanceMap1=getAbundance(imHR,dim)
    imLR=np.zeros(dim)
    downsampleMatrix=np.zeros((imHR.shape[0],imHR.shape[1],imLR.size))
    for i in xrange(dim[0]):
        for j in xrange(dim[1]):
            ...
            imLR[i,j]=np.sum(numba_outer(abundanceMap0[i,max(i*int(np.floor(di))-1,0):(i+1)*int(np.ceil(di))+1],abundanceMap1[j,max(0,j*int(np.floor(dj))-1):(j+1)*int(np.ceil(dj))+1])*imHR[max(0,i*int(np.floor(di))-1):(i+1)*int(np.ceil(di))+1,max(0,j*int(np.floor(dj))-1):(j+1)*int(np.ceil(dj))+1])
    
    downsampleMatrix=downsampleMatrix.reshape(imHR.size,imLR.size)
    #handle nan values
    imLR[np.where(np.isnan(imLR))]=np.median(imLR[np.where(np.isnan(imLR)==False)])
    if returnMatrix==True:
        return imLR,downsampleMatrix,abundanceMap0.T,abundanceMap1.T
    return imLR
