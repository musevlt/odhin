...
def calcCondNumber(debl):
    if debl.background is True:
        mat = np.array(debl.listIntensityMapLRConvol[0][0][:-1])
    else:
        mat = np.array(debl.listIntensityMapLRConvol[0][0])
    for col in mat:
        col[:] = col/np.sum(col)
...
def calcRMSE(listSpe, listSpeRef):
    res=np.mean([np.sqrt(np.mean((spe0-spe1)**2)) for spe0,spe1 in zip(listSpe, listSpeRef)])
    return res
...
def calcRefCorr(debl, simu):
    res0 = np.dot(debl.sources[0], simu.spectraTot[0])/np.linalg.norm(debl.sources[0])/np.linalg.norm(simu.spectraTot[0])
    res1 = np.dot(debl.sources[1], simu.spectraTot[1])/np.linalg.norm(debl.sources[1])/np.linalg.norm(simu.spectraTot[1])
    return res0, res1
...
def calcRefCorr2(listSpe, listSpeRef):
    ...
    for spe0, spe1 in zip(listSpe, listSpeRef):
        listRes.append(np.dot(spe0, spe1)/np.linalg.norm(spe0)/np.linalg.norm(spe1))
    return listRes
