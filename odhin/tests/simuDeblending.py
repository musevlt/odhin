...
class SimuDeblending:
    ...
    def __init__(self, listCenter, listSpectra, listRadius, listIntens,
        ...
        if PSFMuse is None:
            self.PSFMuse = generateMoffatIm(
                shape=(
                    21, 21), center=(
                    10, 10), alpha=4, beta=2.8, dim=None)
        else:
        ...
        self.fwhm_hst = 0.085
        self.betaHST = 1.6
        a_hst = np.sqrt(self.fwhm_hst**2 / 4.0 /
                        (2.0**(1.0 / self.betaHST) - 1.0))
        self.PSFHST = generateMoffatIm(
            shape=(
                101, 101), center=(
                50, 50), alpha=a_hst, beta=self.betaHST, dim='HST')
        self.ImHR2 = ssl.fftconvolve(self.ImHR2, self.PSFHST, mode='same')
        for k in range(self.nbS):
            self.mapAbundances[k] = ssl.fftconvolve(
                self.mapAbundances[k].reshape(
                    self.shapeHR), self.PSFHST, mode='same').flatten()
        ...
        self.subMatrix = np.zeros(
            (self.ImHR.size, self.shapeLR[0] * self.shapeLR[1]))
        ...
        self.mapAbundancesConvol = np.zeros(
            (self.nbS, self.shapeHR[0] * self.shapeHR[1]))
        self.mapAbundancesLRConvol = np.zeros(
            (self.nbS, self.shapeLR[0] * self.shapeLR[1]))

        for k in range(self.nbS):
            self.mapAbundancesConvol[k] = ssl.fftconvolve(
                self.mapAbundances[k].reshape(
                    self.shapeHR), imTransfertHSTMUSE, mode='same').flatten()
            self.mapAbundancesLRConvol[k] = downsampling(
                self.mapAbundancesConvol[k].reshape(
                    self.shapeHR), self.shapeLR).flatten()
        ...
        if genFromCubeHR:
            self.CubeLR, self.downsamplingMatrix, self.Mh, self.Mv = downsampling(
                self.CubeHR, self.shapeLR, returnMatrix=True)  # downsampling
            self.CubeLR = ssl.fftconvolve(
                self.CubeLR, self.PSFMuse[np.newaxis, :, :], mode='same')  # l*n1*n2
        else:
            _, self.downsamplingMatrix, self.Mh, self.Mv = downsampling(
                self.CubeHR[0:1], self.shapeLR, returnMatrix=True)  # downsampling
            self.CubeLR = np.dot(
                self.mapAbundancesLRConvol.T,
                self.DicSources.T).T.reshape(
                self.LBDA,
                self.shapeLR[0],
                self.shapeLR[1])
    ...
    def _generateHSTMUSE_transfert_PSF(self):
        ...
        os.system(
            'astconvolve --kernel=%ssharper.fits --makekernel=%s %swider.fits --output=%skernel_simu.fits' %
            (tmp_dir,
             np.maximum(
                 im.shape[0],
                 im.shape[1]) //
    ...
    def generateImHR(self):
        ...
        self.mapAbundances_hide = np.zeros(
            (self.nbS - len(self.listHiddenSources), self.ImHR.size))
        ...
        for k, center in enumerate(self.listCenter):
            ...
            self.spectraTot[k] = np.sum(
                self.mapAbundances[k]) * self.listSpectra[k]
    ...
    def generateCubeHR(self):
        ...
        for k, spec in enumerate(self.listSpectra):
            cubeHR[:, self.label == k] = spec[:, np.newaxis] * \
                np.tile(self.ImHR[self.label == k], (self.LBDA, 1))
        return cubeHR

    def generatePSFMatrixLR(self):
        shapeFSF = self.PSFMuse.shape

        self.matrixPSF = np.zeros(
            (self.shapeLR[0],
             self.shapeLR[1],
             self.shapeLR[0] *
             self.shapeLR[1]))
        for i in range(self.shapeLR[0]):
            for j in range(self.shapeLR[1]):
                imin = int(max(0, i -shapeFSF[0] //2))
                imax = int(min(self.shapeLR[0], i + shapeFSF[0] //2 + 1))
                imax_psf = int(min(shapeFSF[0], self.shapeLR[0] + shapeFSF[0] * 1/2. - i))
                jmin = int(max(0, j - shapeFSF[1] // 2))
                jmax = int(min(self.shapeLR[1], j + shapeFSF[1] //2 + 1))
                self.matrixPSF[imin:imax, jmin:jmax, j + self.shapeLR[0] * i] = self.PSFMuse[imin:imax_psf, jmin:int(min(shapeFSF[1], self.shapeLR[1] +
                                                                                              shapeFSF[1] *
                                                                                              1 /
                                                                                              2. -
                                                                                              j))]
        self.matrixPSF = self.matrixPSF.reshape(
            (self.shapeLR[0] * self.shapeLR[1], self.shapeLR[0] * self.shapeLR[1]))
...
def generateGaussianIm(center=(12, 12), shape=(25, 25), sig=3.):
    ...
    res = np.exp(- ((ind[0] - center[0])**2 +
                    (ind[1] - center[1])**2) / (2 * sig**2))
    res = res / np.sum(res)
    return res
